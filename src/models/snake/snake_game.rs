...
impl SnakeGame {
    ...
    pub fn new(num_of_bots: usize, num_of_real_players: usize, board_size: usize) -> Result<Self> {
        ...
        if num_players == 1 {
            ...
            for i in 0..num_players {
                ...
                let bx = (start.0 as i64 + i64::from(op.0)) as usize;
                let by = (start.1 as i64 + i64::from(op.1)) as usize;
    ...
    pub fn check_for_winner(&mut self) {
        if self.snakes.len() == 1 {
            if self.snakes.first().unwrap().is_dead() {
                info!("Game over");
                self.winner = Some(self.snakes.first().unwrap().player_id);
            }
            return;
        }
        ...
        if pot_winner.is_some() {
            info!(
                ...
                pot_winner.unwrap().player_id + 1
            );
            self.winner = Some(pot_winner.unwrap().player_id);
        }
    }
