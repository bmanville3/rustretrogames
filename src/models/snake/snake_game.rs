...
impl SnakeGame {
    ...
    pub fn new(num_of_bots: usize, num_of_real_players: usize, board_size: usize) -> Result<Self> {
        ...
        if num_players > MAX_NUM_OF_TOTAL_PLAYERS || num_players < 1 {
        ...
        if num_players == 1 {
            ...
            ns.squares_taken.push_back((player_one_loc - 1, player_one_loc));
            snakes.push(ns);
        } else {
            ...
            for i in 0..num_players {
                ...
                let mut ns = SnakePlayer::new(are_bots[i], start.0, start.1, i, starting_action);
                grid[start.0][start.1] = SnakeBlock::SnakeHead(i);
                // TODO: \/ Make this not terrible \/
                let bx = (start.0 as i64 + op.0 as i64) as usize;
                let by = (start.1 as i64 + op.1 as i64) as usize;
    ...
    fn put_random_apple(&mut self) {
        ...
        if let Some(index) = index {
            ...
            let index = avaliable[avaliable.len() / 2];
            self.grid[index.0][index.1] = SnakeBlock::Apple;
        }
    }

    pub fn check_for_winner(&mut self) {
        // TODO: This is invalid for only single player
        let mut count = 0;
        ...
        if count == 1 {
            info!("Player {} won", pot_winner.unwrap() + 1);
            self.winner = pot_winner;
        }
    }
    ...
    pub fn move_character(
        ...
        if self.winner.is_some() || snake_indx >= self.snakes.len() {
            debug!("Trid to move snake invalidly");
            return false;
        }
