...
impl SnakeViewModel {
    ...
    pub fn make_bot_thread(&self, player_indx: usize) -> impl Stream<Item = Message> {
        let bot = if self.params.bot_type.is_none() {
            error!(
    ...
    fn check_bot_ready(&self, pindx: usize, sender: Sender<ChannelMessage>) -> bool {
        ...
        let player = if let Some(p) = self.model.get_player(pindx) {
            p
        } else {
    ...
    pub fn get_winner(&self) -> Option<&SnakePlayer> {
        self.model.get_winner()?;
        self.model.get_player(self.model.get_winner().unwrap())
    }
    ...
    pub fn real_players_lost(&self) -> bool {
        ...
        let b1 = self.params.number_of_real_players > 0
            && self
                .model
                .get_player(self.real_player_1_index.unwrap())
                .unwrap()
                .is_dead();
        ...
        b1 && self
            .model
            .get_player(self.real_player_2_index.unwrap())
            .unwrap()
            .is_dead()
    }
